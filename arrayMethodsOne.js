...
const postTaxPrices = prices.map((element)=> Math.round(element * 1.07));
...
let orderTotals = orders.map((element) => +(element.price * (element.tax += 1)).toFixed(2))
...
let bobsTotal = purchases.filter((element => element.owner === 'Bob')).reduce((acc, curr)=> acc + curr.price,0)
